...
exports.getInventoryTurnover = catchAsync(async (req, res, next) => {
    ...
    const inventoryTurnover = await Invoice.aggregate([
        // 1. Start with Invoices, match by owner and date first
        { $match: { ...ownerFilter, invoiceDate: { $gte: start, $lte: end } } },
        
        // 2. Unwind the items array to process each product sale individually
        { $unwind: "$items" },
        
        // 3. Group by product to get total sold quantity in the period
        {
            $group: {
                _id: "$items.product",
        ...
        // 4. Now, lookup the product details for each sold item
        {
            $lookup: {
                from: "products",
                localField: "_id",
        ...
        // 5. Project the final shape and calculate the turnover rate
        {
            $project: {
                _id: 1,
                ...
                turnoverRate: {
                    $cond: {
                        ...
                        then: { $divide: ["$totalSold", "$productDetails.stock"] },
                        else: 0 // If stock is 0, turnover is effectively infinite, but 0 is a safe value
                    }
                }
            }
        },
    ...
    res.status(200).json({ status: 'success', data: inventoryTurnover });
});