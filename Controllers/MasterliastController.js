...
const getOwnerFilter = (user) => {
    if (user && user.role === 'superAdmin') {
        return {}; // Super admins have no owner restrictions
    }
    return { owner: user ? user._id : null };
};

exports.getMasterList = catchAsync(async (req, res, next) => {
    const ownerFilter = getOwnerFilter(req.user);

    // OPTIMIZATION: Added .lean() to all queries for a major performance boost.
    const promises = [
        ...
        Payment.find(ownerFilter).select('customerId customerName phoneNumbers _id').lean(),
