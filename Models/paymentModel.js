...
const paymentSchema = new Schema(
    {
        ...
        // --- NEW FIELD TO LINK TO EMI ---
        emiInfo: {
            emiId: { type: Schema.Types.ObjectId, ref: "Emi" },
...
// THIS HOOK WILL NOW AUTOMATICALLY RECALCULATE THE CUSTOMER'S BALANCE
paymentSchema.post("save", async function (doc) {
    const Customer = mongoose.model("Customer");
    const customer = await Customer.findById(doc.customerId);
    if (!customer) return;

    if (!customer.paymentHistory.includes(doc._id)) {
        customer.paymentHistory.push(doc._id);
        await customer.save();
    }

    await Customer.recalculateBalances(doc.customerId);
});
...
//     owner: { type: Schema.Types.ObjectId, ref: 'User', required: true },
//     amount: { type: Number, required: true, min: 0 },
//     paymentMethod: { type: String, required: true, enum: ['credit_card', 'debit_card', 'upi', 'crypto', 'bank_transfer'] },
