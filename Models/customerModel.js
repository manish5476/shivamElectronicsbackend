...
const customerSchema = new Schema({
  ...
  mobileNumber: { 
    type: String, 
    required: function() { return !this.phoneNumbers.length; },  // Mobile number is required if no phone numbers exist
    match: /^\d{10}$/,  // Optional: Validate 10 digit phone number
  }, 
  phoneNumbers: {
    type: [
      {
...
customerSchema.pre('save', function(next) {
    if (this.phoneNumbers && this.phoneNumbers.length > 0 && !this.mobileNumber) {
      // Set the first phone number as the mobileNumber
      this.mobileNumber = this.phoneNumbers[0].number;
    }
    next();
  });
  customerSchema.virtual('calculatedMobileNumber').get(function() {
    if (this.phoneNumbers && this.phoneNumbers.length > 0) {
      return this.phoneNumbers[0].number; // Return the first phone number as the "mobileNumber"
    }
    return null;
  });
...
customerSchema.statics.calculateTotalPurchasedAmount = async function (customerId) {
  ...
  customer.totalPurchasedAmount = result[0]?.totalAmount || 0;
  await customer.save();
  return customer.totalPurchasedAmount;
};

customerSchema.statics.updateRemainingAmount = async function (customerId) {
  const customer = await this.findById(customerId).populate('paymentHistory');
  if (!customer) return null;
  ...
  await customer.save();
  return customer.remainingAmount;
};
