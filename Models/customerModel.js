...
const customerSchema = new Schema({
    ...
    cart: {
        items: [
            { productId: { type: Schema.Types.ObjectId, ref: 'Product' }, invoiceIds: [{ type: Schema.Types.ObjectId, ref: 'Invoice' }] }
        ],
    },
...
customerSchema.pre(/^find/, function (next) {
    this.populate([
        { path: 'cart.items.productId', select: 'title price thumbnail' },
        { path: 'cart.items.invoiceIds', select: 'invoiceNumber totalAmount invoiceDate status' },
        { path: 'paymentHistory', select: 'amount status createdAt transactionId' }
    ]);
    next();
});

// --- Centralized balance calculation ---
customerSchema.statics.recalculateBalances = async function (customerId) {
    const Invoice = mongoose.model("Invoice");
    const Payment = mongoose.model("Payment");

    const [invoices, payments] = await Promise.all([
        Invoice.aggregate([
            { $match: { buyer: customerId, status: { $ne: "cancelled" } } },
    ...
    const totalPurchasedAmount = invoices.length ? invoices[0].total : 0;
    const totalPaid = payments.length ? payments[0].total : 0;
    const remainingAmount = Math.max(totalPurchasedAmount - totalPaid, 0);
